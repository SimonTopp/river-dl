...
rule all:
    input:
        expand("results/{outdir}/RGCN/{train_type}/rep_{replicate}/{metric_type}_metrics.csv",
            out_main=out_main,
        ...
        expand("results/asRunConfig_RGCN.yml",model=model),
        expand("results/Snakefile_RGCN.smk",model=model)


## Log config file
rule as_run_config:
    output:
        "results/asRunConfig_RGCN.yml"
    group: "prep"
    run:
...
rule prep_io_data:
    ...
    output:
        "results/{outdir}/RGCN/prepped.npz"
    threads: 2
...
rule pre_train:
    input:
        "results/baseline/RGCN/prepped.npz"
    output:
        "results/baseline/RGCN/rep_pre_{replicate}/pretrained_weights.pth",
        "results/baseline/RGCN/rep_pre_{replicate}/pretrain_log.csv"
    threads: 4
    group: 'pre_train'
    run:
        ...
        model = RGCN_v1(in_dim,config['hidden_size'],adj_mx,device=device,recur_dropout=config['recurrent_dropout'])
        ...
        train_torch(model,
            ...
            log_file=output[1],
            device=device,
            keep_portion=config['trn_offset'],
            shuffle=False)
...
rule finetune_train_full:
    input:
        expand("results/baseline/RGCN/{rep}/pretrained_weights.pth",
            rep=[f"rep_pre_{i}" for i in replicates]),
        expand("results/baseline/RGCN/{rep}/pretrain_log.csv",
            rep=[f"rep_pre_{i}" for i in replicates]),
        "results/{outdir}/RGCN/prepped.npz"

    output:
        "results/{outdir}/RGCN/full_train/rep_{replicate}/finetuned_weights.pth",
        "results/{outdir}/RGCN/full_train/rep_{replicate}/finetune_log.csv"
    threads: 4
    group: 'train'
    run:
        os.system("module load analytics cuda11.3/toolkit/11.3.0")
...
rule finetune_train_no_pt:
    ...
    output:
        "results/{outdir}/RGCN/no_pt/rep_{replicate}/finetuned_weights.pth",
        "results/{outdir}/RGCN/no_pt/rep_{replicate}/finetune_log.csv"
    threads: 4
    group: 'train'
    run:
        os.system("module load analytics cuda11.3/toolkit/11.3.0")
...
rule finetune_train_reset_adj:
    ...
    output:
        "results/{outdir}/RGCN/reset_spatial/rep_{replicate}/finetuned_weights.pth",
        "results/{outdir}/RGCN/reset_spatial/rep_{replicate}/finetune_log.csv"
    threads: 4
    group: 'train'
    run:
        os.system("module load analytics cuda11.3/toolkit/11.3.0")
        ...
        ## Re-initialize spatial weights
        torch.nn.init.xavier_uniform_(model.weight_q)
...
rule make_predictions:
    input:
        "results/{outdir}/RGCN/{train_type}/rep_{replicate}/finetuned_weights.pth",
        "results/{outdir}/RGCN/prepped.npz"
    output:
        "results/{outdir}/RGCN/{train_type}/rep_{replicate}/{partition}_preds.feather",
    group: 'train'
    threads: 3
    run:
        os.system("module load analytics cuda11.3/toolkit/11.3.0")
...
rule combine_metrics:
    input:
        config['obs_file'],
        "results/{outdir}/RGCN/{train_type}/rep_{replicate}/trn_preds.feather",
        "results/{outdir}/RGCN/{train_type}/rep_{replicate}/val_preds.feather",
        "results/{outdir}/RGCN/{train_type}/rep_{replicate}/tst_preds.feather"
    output:
        "results/{outdir}/RGCN/{train_type}/rep_{replicate}/{metric_type}_metrics.csv"
    group: 'train'
    threads: 3
    params:
        grp_arg=get_grp_arg,
