...
rule all:
    input:
        expand("results/xai_outputs/noise_annual/{model}_{run}_reach_noise.csv",
            model=['GWN','RGCN'],
            run=['pt','nopt','ptft','coast_ptft', 'head_nopt','head_ptft','coast_nopt', 'rs_adj'],
        ),
        expand("results/xai_outputs/noise_seasonal/{model}_{run}_seasonal_noise.csv",
            model=['GWN','RGCN'],
            run=['pt','nopt','ptft']#,'coast_ptft', 'head_nopt','head_ptft','coast_nopt', 'rs_adj'],
        )
...
rule gwn_reach_noise:
    ...
    output:
        "results/xai_outputs/noise_annual/{model}_{run}_reach_noise.csv"
    ...
    run:
        ...
        diffs_df.to_csv(f"results/xai_outputs/noise_annual/GWN_{wildcards.run}_reach_noise.csv",
                        index=False)
...
rule rgcn_reach_noise:
    ...
    run:
        ...
        diffs_df.to_csv(f"results/xai_outputs/noise_annual/RGCN_{wildcards.run}_reach_noise.csv",
                        index=False)
...
rule gwn_seasonal_noise:
    input:
        f"results/baseline/GWN/prepped.npz",
    output:
        "results/xai_outputs/noise_seasonal/{model}_{run}_seasonal_noise.csv"
    wildcard_constraints:
        model='GWN'
    params: weights = return_weights_file
    run:
        ...
        out.to_csv(f"results/xai_outputs/noise_seasonal/GWN_{wildcards.run}_seasonal_noise.csv",
                        index=False)
...
rule rgcn_seasonal_noise:
    ...
    run:
        ...
        out.to_csv(f"results/xai_outputs/noise_seasonal/RGCN_{wildcards.run}_seasonal_noise.csv",
                        index=False)
